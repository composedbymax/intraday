...
function doChartData(){
  ...
  $bars=(int)($_GET['bars']??0);
  $direction=$_GET['direction']??($bars>0?'before':'');
  ...
  if($bars>0){
    if(!in_array($direction,['before','after'],true))$direction='before';
    [$data,$endOfData]=loadHistoricalBars($pdo,$sym,$int,$bars,$direction,$anchor?:($direction==='before'?time():0));
    if($limit!==null&&count($data)>$limit)$data=$direction==='before'?array_slice($data,-$limit):array_slice($data,0,$limit);
    $first=$data[0]['time']??($anchor?:0);
    $last=$data[count($data)-1]['time']??($anchor?:0);
    ok(['candles'=>$data,'symbol'=>$sym,'interval'=>$int,'requestedBars'=>$bars,'loadedBars'=>count($data),'end_of_data'=>$endOfData,'p1'=>$first,'p2'=>$last]);
  }
  ...
  if($limit!==null&&count($data)>$limit){$data=array_slice($data,-$limit);$p1=$data[0]['time'];}
  ok(['candles'=>$data,'symbol'=>$sym,'interval'=>$int,'loadedBars'=>count($data),'end_of_data'=>false,'p1'=>$p1,'p2'=>$p2]);
}
function doSearch(){
  $q=trim($_GET['q']??'');
  if(!$q)err('No query');
...
function doSetTrack(){
  ...
  $pdo->prepare("INSERT INTO tracked_assets(user,symbol,`interval`,auto_update_enabled)VALUES(?,?,?,?)ON DUPLICATE KEY UPDATE auto_update_enabled=?")->execute([$user,$sym,$int,$enabled,$enabled]);
  ok(['ok'=>true]);
}
...
function doCheckUpdates(){
  ...
  if(!$sym||!$int)err('Missing params');
  $data=fetchYahoo($sym,$int,$since+1,time());
  if($data)storeCandles($pdo,$sym,$int,$data);
  $data=getCachedCandles($pdo,$sym,$int,$since+1,time());
  ok(['candles'=>$data,'loadedBars'=>count($data)]);
}